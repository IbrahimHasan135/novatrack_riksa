        ...
        <div class="acct-grid">
            <article class="acct-card span-5">
                ...
                <form class="acct-form" action="<?= app_url('accounting/income'); ?>" method="POST">
                    ...
                    <div class="acct-field full acct-submit-row"><button class="acct-btn secondary" type="submit" name="record_state" value="draft"><i class="bi bi-file-earmark"></i> Simpan Draft</button><button class="acct-btn secondary" type="submit" name="record_state" value="verification"><i class="bi bi-hourglass-split"></i> Verification</button><button class="acct-btn" type="submit" name="record_state" value="published"><i class="bi bi-save"></i> Publish Pemasukan</button></div>
                </form>
            </article>
            ...
            <article class="acct-card"><h2>Riwayat Pemasukan</h2><div class="acct-table-wrap"><table class="acct-table"><thead><tr><th>Tanggal</th><th>Judul</th><th>Sumber</th><th>Client</th><th>Nominal</th><th>Ref</th><th>State</th><th></th></tr></thead><tbody><?php foreach ($rows as $row): ?><tr><td><?= nt_date($row['received_date']); ?></td><td><strong><?= htmlspecialchars($row['title']); ?></strong><br><span><?= htmlspecialchars($row['payment_method']); ?></span></td><td><?= htmlspecialchars($row['source_name'] ?? 'Uncategorized'); ?></td><td><?= htmlspecialchars($row['client_name']); ?></td><td><strong><?= nt_money($row['amount']); ?></strong></td><td><?= htmlspecialchars($row['reference_no']); ?></td><td><span class="acct-pill <?= nt_record_state_class($row['record_state'] ?? 'published'); ?>"><?= htmlspecialchars(nt_record_state_label($row['record_state'] ?? 'published')); ?></span></td><td class="acct-row-actions"><a class="acct-edit" href="<?= app_url('accounting/income/edit/' . (int)$row['id']); ?>"><i class="bi bi-pencil-square"></i></a><form action="<?= app_url('accounting/income/delete/' . (int)$row['id']); ?>" method="POST" onsubmit="return confirm('Hapus pemasukan ini?');"><button class="acct-delete" type="submit"><i class="bi bi-trash"></i></button></form></td></tr><?php endforeach; ?></tbody></table></div></article>
        </div>
    </section>
</main>
