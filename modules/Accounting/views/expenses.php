    ...
    <section class="acct-shell">
        ...
        <div class="acct-grid">
            <article class="acct-card span-5"><h2>Tambah Pengeluaran</h2><form class="acct-form" action="<?= app_url('accounting/expenses'); ?>" method="POST">
                ...
                <div class="acct-field full"><button class="acct-btn" type="submit"><i class="bi bi-save"></i> Simpan Pengeluaran</button></div>
            </form></article>
            <article class="acct-card span-7"><h2>Pengeluaran Terbesar Bulan Ini</h2><div class="acct-list"><?php $max = max(1, ...array_map(fn($r) => (float)$r['total'], $categoryStats ?: [['total' => 1]])); ?><?php foreach ($categoryStats as $cat): ?><div class="acct-list-row"><div style="flex:1;"><b><?= htmlspecialchars($cat['category_name']); ?></b><br><span><?= (int)$cat['transactions']; ?> transaksi</span><div class="acct-progress"><span style="width:<?= min(100, ((float)$cat['total'] / $max) * 100); ?>%"></span></div></div><strong><?= nt_money($cat['total']); ?></strong></div><?php endforeach; ?></div></article>
            <article class="acct-card"><h2>Riwayat Pengeluaran</h2><div class="acct-table-wrap"><table class="acct-table"><thead><tr><th>Tanggal</th><th>Judul</th><th>Kategori</th><th>Vendor</th><th>Nominal</th><th>Ref</th><th></th></tr></thead><tbody><?php foreach ($rows as $row): ?><tr><td><?= nt_date($row['expense_date']); ?></td><td><strong><?= htmlspecialchars($row['title']); ?></strong><br><span><?= htmlspecialchars($row['payment_method']); ?></span></td><td><?= htmlspecialchars($row['category_name'] ?? 'Uncategorized'); ?></td><td><?= htmlspecialchars($row['vendor_name']); ?></td><td><strong><?= nt_money($row['amount']); ?></strong></td><td><?= htmlspecialchars($row['reference_no']); ?></td><td class="acct-row-actions"><a class="acct-edit" href="<?= app_url('accounting/expenses/edit/' . (int)$row['id']); ?>"><i class="bi bi-pencil-square"></i></a><form action="<?= app_url('accounting/expenses/delete/' . (int)$row['id']); ?>" method="POST" onsubmit="return confirm('Hapus pengeluaran ini?');"><button class="acct-delete" type="submit"><i class="bi bi-trash"></i></button></form></td></tr><?php endforeach; ?></tbody></table></div></article>
        </div>
    </section>
</main>
