...
<main class="main-content">
    <section class="sales-shell">
        ...
        <div class="sales-grid">
            ...
            <article class="sales-card span-7"><h2>Pipeline by Stage</h2><div class="sales-list"><?php $max = max(1, ...array_map(fn($r) => (float)$r['value'], $pipeline ?: [['value' => 1]])); ?><?php foreach ($pipeline as $row): ?><div class="sales-list-row"><div style="flex:1;"><b><?= htmlspecialchars(sales_label($row['stage'])); ?></b><br><span><?= (int)$row['total']; ?> opportunity · <?= sales_money($row['value']); ?></span><div class="sales-progress"><span style="width:<?= min(100, ((float)$row['value'] / $max) * 100); ?>%"></span></div></div></div><?php endforeach; ?></div></article>
